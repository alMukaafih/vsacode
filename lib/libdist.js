 ...
 * @param {string} id - Icon Theme id
 * @param {string} req - Name of required File
 * @param {string} fallback - Fallback to default to
 * @returns {string} Path of required File
 */
function check(id, req, fallback) {
    if (!reqExists(id, req))
        return path.join(includes, fallback[req]);
...
function include(id, fallback) {
    let plugin_json = check(id, "plugin.json");
    let readme_md = check(id, "readme.md");
    let icon_png = check(id, "icon.png");
 ...
 * @returns {void}
 */
function distBuild(label, id, acode, fallback) {
    let owd = process.cwd();
    process.chdir(acode);
